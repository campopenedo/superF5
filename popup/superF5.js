...
document.getElementById("notification-ok").addEventListener("click", closeAndHidePopup);
...
function showPopup(message) {
    let popup = document.getElementById("notification-message"),
    okButton = document.getElementById("notification-ok");
    
    var messageNode = document.createTextNode(message);
    var wrapper = document.createElement("span");
    wrapper.id = "notification-message-text"
    wrapper.appendChild(messageNode);

    popup.insertBefore(wrapper, okButton);

    popup.parentElement.classList.remove("hidden");
}

function closeAndHidePopup() {
    document.getElementById("notification-message-text").remove();
